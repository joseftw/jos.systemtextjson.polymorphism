...
using JOS.SystemTextJsonPolymorphism.PolymorphicCovariant.Car;
using JOS.SystemTextJsonPolymorphism.PolymorphicCovariant.Truck;
using Shouldly;
using Xunit;
...
namespace JOS.SystemTextJsonPolymorphism.Tests
{
    public class PolymorphicCovariantTests
    {
        ...
        public async Task ShouldDeserializeVehiclesCorrectly()
        {
            ...
            {
                var result = await JsonSerializer.DeserializeAsync<List<PolymorphicCovariant.Vehicle>>(jsonFile, DefaultJsonSerializerOptions.Instance);
